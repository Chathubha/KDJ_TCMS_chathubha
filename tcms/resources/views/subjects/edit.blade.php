<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-gray-900">Edit Subject: {{ $subject->name }}</h2>
    </x-slot>
            ...
            <form method="POST" action="{{ route('subjects.update', $subject) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none">{{ old('description', $subject->description) }}</textarea>
                </div>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" class="bg-gray-900 hover:bg-gray-800 text-white font-bold py-3 px-6 rounded-xl transition shadow-sm">Update Subject</button>
                </div>
            </form>
        </div>
    </div>
