<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <h2 class="text-2xl font-bold text-gray-900">Edit Student: {{ $student->full_name }}</h2>
    ...
    <div class="max-w-3xl space-y-6">
        <form method="POST" action="{{ route('students.update', $student) }}">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5 mt-6">
                ...
                <div>
                    ...
                    <textarea name="address" rows="3"
                              class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none resize-none">{{ old('address', $student->address) }}</textarea>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 mt-6">
                ...
                <button type="submit" class="bg-gray-900 hover:bg-gray-800 text-white font-bold py-3 px-6 rounded-xl transition shadow-sm">Update Student</button>
            </div>
        </form>
    </div>
</x-app-layout>
