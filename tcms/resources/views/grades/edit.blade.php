<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-gray-900">Edit Exam: {{ $exam->name }}</h2>
    </x-slot>

    <div class="max-w-2xl space-y-6">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <form method="POST" action="{{ route('grades.update', $exam) }}" class="space-y-6">
                @csrf @method('PUT')

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Exam Name *</label>
                    <input type="text" name="name" value="{{ old('name', $exam->name) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none" required>
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="date" name="date" value="{{ old('date', $exam->date?->format('Y-m-d')) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none">
                    ...
                    <div>
                        ...
                        <input type="number" name="max_marks" value="{{ old('max_marks', $exam->max_marks) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none" min="1" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="weight" value="{{ old('weight', $exam->weight) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none" min="1" required>
                ...
                <div class="flex justify-end space-x-3 pt-4">
                    ...
                    <button type="submit" class="bg-gray-900 hover:bg-gray-800 text-white font-bold py-3 px-6 rounded-xl transition shadow-sm">Update Exam</button>
                </div>
            </form>
        </div>
    </div>
